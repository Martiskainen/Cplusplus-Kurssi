...
bool is_valid_phone_number(const string number) {
    for( unsigned int i = 0; i < number.length(); ++i ) {
        if( not(('0' <= number[i] and number[i] <= '9')
            ...
            cout << "\nErroneous phone number: " << number << endl;
            return false;
        }
    }
    return true;
...
void update_file(const string& file_name,
                 const map<string, shared_ptr<Student>>& user_ids) {
    ...
    // Write the data from the numerical order map to the file
    for (const auto& pair : user_ids) {
        const Student& s = *(pair.second);
        new_file << s.student_number << ";"
                    << s.user_id << ";"
                    << s.name << ";"
                    << s.phone_number << ";"
                    << s.email << ";"
            ...
            if( parts.size() != 2 ){
                ...
                for( auto& pair: student_numbers ) {
                    if ((*(pair.second)).student_number == parts.at(1)) {
                        if (change_phone_number(*(pair.second))) {

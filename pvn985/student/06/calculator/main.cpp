...
bool execute_command(vector<string>& command_line)
{
    string command_to_be_executed = command_line.at(0);
    ...
    {
        // löytyikö oikea komento
        // command = struct in vector of structs
        if (command_to_be_executed != command.str)
        {
            continue;
        }
        // Tarksita parametrien määrä
        if (command_line.size() - 1 != command.parameter_number)
        {
            cout << "Error: wrong number of parameters." << endl;
            return true;
        }
        // Onko kyseessä lopetuskomento
        if (command.is_exit)
        {
            cout << GREETING_AT_END << endl;
            return false;
        }
        // Tarkista, ovatko parametrit double-muotoisia (string_to_double)
        double operand1, operand2;
        if (!string_to_double(command_line[1], operand1) || !string_to_double(command_line[2], operand2))
        {
            cout << "Error: a non-number operand." << endl;
            return true;
        }
        // Kutsu laskutoimitus
        double result = command.action(operand1, operand2);
